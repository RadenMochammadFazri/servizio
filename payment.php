...
if (isset($_POST['bayar'])) {
  ...
  $metode =$_POST['metode'];
  $id_produk=$_GET['id'];
  // $select = "SELECT produk.id_produk, produk.gambar_produk1 , cart.id_produk, cart.harga_produk FROM produk JOIN cart ON (produk.id_produk=cart.id_produk)
  // WHERE cart.id_produk='$id_produk'";
  $pilih = "SELECT produk.id_produk, gambar_produk1, produk.harga_produk FROM produk 
...
$numberString = preg_replace('/[^0-9]/', '', $price); // Menghapus semua karakter non-angka dari string
$number = intval($numberString); // Mengkonversi string menjadi bilangan bulat (integer)
  
  
  $var = "INSERT INTO proses (invoice, status_proses, id_produk, gambar,metode, total_bayar)
                VALUES ('$invoice','Selesai','$id_produk', '$image','$metode', '$number')";
                
                
              $hasil = $conn -> query($var);
        
              if($hasil === TRUE){
                header("Location: order_complete.php?id=$id_produk");
              } else{ 
                echo"gagal!";
              }
}
...
<html lang="en">
  ...
  <body class="bg-darkest font-poppins">
    ...
    <header class="py-4 shadow-sm bg-darkest">
      <div class="container flex items-center justify-between">
        ...
        <div class="space-x-6 flex items-center">
          <a href="#" class="lg:block text-center text-white hover:text-primary transition hidden relative">
    ...
    <!-- account wrapper -->
    <div class="container lg:grid grid-cols-12 items-start gap-6 pt-16">
      <!-- sidebar -->
      <div class="col-span-3">
        ...
        <div class="mt-6 bg-dark shadow rounded p-4 divide-y divide-gray-200 space-y-4 text-gray-600">
          ...
          <div class="space-y-1 pl-8 pt-4">
            <a href="#" class="relative medium capitalize text-primary font-medium hover:text-primary transition block">
              My order history
              <span class="absolute -left-8 top-0 text-base text-white">
        ...
        <?php
            ...
            $image = $baris['gambar_produk1'];
              ...
              <form action='' method='POST'>
                <h3 class='font-medium capitalize mb-4 text-white text-center text-2xl'>Confirm Payment</h3>
                <div class='space-y-4'>
                  <div class='grid sm:grid-cols-2 gap-4'>
                    <div class='mt-6'>
                      <label class='block text-white' for=''>Invoice Number </label>
                      <input type='text' name='invoice' class='text-white bg-dark w-full mt-2 px-4 py-3 border border-gray-200 focus:border-[#00ADB5] focus:bg-darkest focus:outline-none rounded-lg' />
                    </div>
                    <div class='mt-6'>
                      <label class='block text-white' for=''>Amount </label>
                      <input type='text' name='price' value='$price' class='text-white bg-dark w-full mt-2 px-4 py-3 border border-gray-200 focus:border-[#00ADB5] focus:bg-darkest focus:outline-none rounded-lg' />
                    </div>
                  </div>
                  ...
                  <div class='grid sm:grid-cols-2 gap-4 pt-2'>
                    <!-- payment 1 start  -->
                    <div class='relative'>
                      <input type='radio' name='metode' id='payment1' class='hidden peer' value='BCA'/>
                      <label
                        for='payment1'
                        class='flex items-center gap-4 p-4 rounded-xl bg-dark border border-white hover:border-primary bg-opacity-90 backdrop-blur-2xl shadow-xl hover:bg-opacity-75 peer-checked:bg-primary peer-checked:text-white cursor-pointer transition'>
                        <img src='/servizio/dist/images/logobca.png' alt='payment photo' class='w-30 h-10 object-cover' />
                      </label>
                      <div class='flex absolute top-0 right-4 bottom-0 w-7 h-7 my-auto rounded-full bg-[#49C2C8] scale-0 peer-checked:scale-100 transition delay-100'>
                        <svg xmlns='http://www.w3.org/2000/svg' fill='currentColor' class='w-5 text-white my-auto mx-auto' viewBox='0 0 16 16'>
                    ...
                    <!-- payment 2 start  -->
                    <div class='relative'>
                      <input type='radio' name='metode' id='payment2' class='hidden peer' value='GOPAY' />
                      <label
                        for='payment2'
                        class='flex items-center gap-4 p-4 rounded-xl bg-dark border border-white hover:border-primary bg-opacity-90 backdrop-blur-2xl shadow-xl hover:bg-opacity-75 peer-checked:bg-primary peer-checked:text-white cursor-pointer transition'  >
                        <img src='/servizio/dist/images/gopay.png' alt='payment photo' class='w-30 h-10 object-cover' />
                      </label>
                      <div class='flex absolute top-0 right-4 bottom-0 w-7 h-7 my-auto rounded-full bg-[#49C2C8] scale-0 peer-checked:scale-100 transition delay-100'>
                        <svg xmlns='http://www.w3.org/2000/svg' fill='currentColor' class='w-5 text-white my-auto mx-auto' viewBox='0 0 16 16'>
                          <path d='M12.736 3.97a.733.733 0 0 1 1.047 0c.286.289.29.756.01 1.05L7.88 12.01a.733.733 0 0 1-1.065.02L3.217 8.384a.757.757 0 0 1 0-1.06.733.733 0 0 1 1.047 0l3.052 3.093 5.4-6.425a.247.247 0 0 1 .02-.022Z' />
                    ...
                    <!-- payment 3 start  -->
                    <div class='relative'>
                      <input type='radio' name='metode' id='payment3' class='hidden peer' value='MANDIRI'/>
                      <label
                        for='payment3'
                        class='flex items-center gap-4 p-4 rounded-xl bg-dark border border-white hover:border-primary bg-opacity-90 backdrop-blur-2xl shadow-xl hover:bg-opacity-75 peer-checked:bg-primary peer-checked:text-white cursor-pointer transition'  >
                        <img src= '/servizio/dist/images/mandiri.png' alt='payment photo' class='w-36 h-10 object-cover' />
                      </label>
                      <div class='flex absolute top-0 right-4 bottom-0 w-7 h-7 my-auto rounded-full bg-[#49C2C8] scale-0 peer-checked:scale-100 transition delay-100'>
                        <svg xmlns='http://www.w3.org/2000/svg' fill='currentColor' class='w-5 text-white my-auto mx-auto' viewBox='0 0 16 16'>
                          <path d='M12.736 3.97a.733.733 0 0 1 1.047 0c.286.289.29.756.01 1.05L7.88 12.01a.733.733 0 0 1-1.065.02L3.217 8.384a.757.757 0 0 1 0-1.06.733.733 0 0 1 1.047 0l3.052 3.093 5.4-6.425a.247.247 0 0 1 .02-.022Z' />
                    ...
                    <!-- payment 4 start  -->
                    <div class='relative'>
                      <input type='radio' name='metode' id='payment4' class='hidden peer' value='OVO' />
                      <label
                        for='payment4'
                        class='flex items-center gap-4 p-4 rounded-xl bg-dark border border-white hover:border-primary bg-opacity-90 backdrop-blur-2xl shadow-xl hover:bg-opacity-75 peer-checked:bg-primary peer-checked:text-white cursor-pointer transition'>
                        <img src='/servizio/dist/images/ovo.png' alt='payment photo' class='w-30 h-10 object-cover' />
                      </label>
                      <div class='flex absolute top-0 right-4 bottom-0 w-7 h-7 my-auto rounded-full bg-[#49C2C8] scale-0 peer-checked:scale-100 transition delay-100'>
                        <svg xmlns='http://www.w3.org/2000/svg' fill='currentColor' class='w-5 text-white my-auto mx-auto' viewBox='0 0 16 16'>
                          <path d='M12.736 3.97a.733.733 0 0 1 1.047 0c.286.289.29.756.01 1.05L7.88 12.01a.733.733 0 0 1-1.065.02L3.217 8.384a.757.757 0 0 1 0-1.06.733.733 0 0 1 1.047 0l3.052 3.093 5.4-6.425a.247.247 0 0 1 .02-.022Z' />
                ...
                <div class='mt-6'>
                  <button type='submit' name='bayar' class='px-6 py-2 text-center text-white bg-primary border border-primary rounded hover:bg-transparent hover:text-primary transition uppercase font-roboto font-medium'>Confirm</button>
                </div>
              </form>";
              ?>
