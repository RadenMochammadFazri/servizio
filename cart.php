...
<html lang="en">
  ...
  <nav class="w-full fixed left-0 top-0 z-50 nav">
    ...
    <!-- cart wrapper -->
    <div class="container lg:grid grid-cols-12 gap-6 items-start pb-24 pt-16 top-32 relative">
      <!-- product cart -->
      <div class="xl:col-span-9 lg:col-span-8">
        <!-- cart title -->
        <div class="bg-dark py-2 mb-4 justify-around pr-12 hidden md:flex ">
          <p class="text-gray-200 text-center">Product</p>
          <p class="text-gray-200 text-center pr-12">Total</p>
        </div>
        <!-- cart title end -->
        ...
        <div class="space-y-4">
          <!-- single cart -->
          <div class="flex items-center md:justify-between gap-4 md:gap-6 p-4 border border-gray-400 rounded flex-wrap md:flex-nowrap">
            <!-- cart image -->
            <?php
            $id_produk=$_GET['id'];
            $select = "SELECT kategori.id_kategori, kategori.kategori, produk.id_produk, produk.judul_produk, produk.harga_produk, produk.gambar_produk1
            FROM produk JOIN kategori ON (produk.id_kategori=kategori.id_kategori) WHERE produk.id_produk='$id_produk'";
            ...
            while ($baris=$hasil->fetch_assoc()) {
                ...
                $insert = "INSERT into cart (jumlah, id_produk, harga_produk, kategori) VALUES (1,'$id_produk','$price', '$kategori')";
            ...
            <div class='md:w-1/3 w-full'>
              <h2 class='text-white mb-3 xl:text-xl textl-lg font-medium uppercase'>$produk</h2>
            </div>
            <!-- cart content end -->
            ...
            <div class='text-white hover:text-primary cursor-pointer'>
            <a href='delete_cart.php?id=$id_produk'>
      ...
      <?php
            $id_produk=$_GET['id'];
            $select = "SELECT produk.id_produk, produk.harga_produk
            ...
            while ($baris=$hasil->fetch_assoc()) {
                $price=$baris['harga_produk'];
                echo"
      <div class='xl:col-span-3 lg:col-span-4 border border-b-gray-400 px-4 py-4 rounded mt-6 lg:mt-0'>
        <h4 class='text-white text-lg mb-4 font-medium uppercase'>ORDER SUMMARY</h4>
        <div class='space-y-1 text-white pb-3 border-b border-gray-200'>
        ...
        <div class='flex justify-between my-3 text-white font-semibold uppercase'>
          <h4>Total</h4>
          <h4>$price</h4>
        </div>
        ...
        <!-- searchbar -->
        <div class='flex mb-5'>
          <input type='text' class='pl-4 w-full border border-r-0 border-primary py-2 px-3 rounded-l-md focus:ring-primary focus:border-primary text-sm text-white bg-darkest' placeholder='Coupon' />
          <button type='submit' class='bg-primary border border-primary text-white px-5 font-medium rounded-r-md hover:bg-transparent hover:text-primary transition text-sm font-roboto'>Apply</button>
        </div>
        <!-- searchbar end -->

        <!-- checkout -->
        <a href='payment.php?id=$id_produk' class='bg-primary border border-primary text-white px-4 py-3 font-medium rounded-md uppercase hover:bg-transparent hover:text-primary transition text-sm w-full block text-center'>
          Process to checkout
        </a>
        <!-- checkout end -->
      </div>
